...
- **Deployed and live (Phase 3 done).** Backend runs on **Render** as a Docker container (health check `/health`, env vars set in the dashboard, `alembic upgrade head` not wired into startup — the live Neon DB was already migrated); frontend runs on **Vercel** at `https://lens-research.vercel.app` (Root Directory `frontend`, Framework Preset Next.js, auto-deploys on push to `main`). Backend `FRONTEND_URL` + `CLERK_AUTHORIZED_PARTIES` point at the Vercel URL for CORS + Clerk `azp`. Verified end-to-end: sign-in → query → report → follow-up all work in production. Next step: **Phase 4 CI** (GitHub Actions: `ruff` + `pytest` on push, Render auto-deploy on merge). Deploy still on Clerk **dev** keys (private/recorded-demo path) and Tavily `search_depth="basic"`.
...
17. Docker + Render backend deploy + Vercel frontend deploy (done — live, verified end-to-end); GitHub Actions CI still pending (Phase 4)
