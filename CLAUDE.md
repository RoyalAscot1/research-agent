...
- **Deployed and live (Phase 3 done).** Backend runs on **Render** as a Docker container (health check `/health`, env vars set in the dashboard, `alembic upgrade head` not wired into startup — the live Neon DB was already migrated); frontend runs on **Vercel** at `https://lens-research.vercel.app` (Root Directory `frontend`, Framework Preset Next.js, auto-deploys on push to `main`). Backend `FRONTEND_URL` + `CLERK_AUTHORIZED_PARTIES` point at the Vercel URL for CORS + Clerk `azp`. Verified end-to-end: sign-in → query → report → follow-up all work in production. **Phase 4 CI in progress** — GitHub Actions workflow added (`.github/workflows/ci.yml`): two parallel jobs (`lint` = `ruff check` + `ruff format --check`; `test` = `pytest`), Python 3.12 to match the Dockerfile, pip caching, `concurrency` cancel-in-progress; triggers on PRs into `main` and pushes to any branch. All three checks verified green locally before commit. Still pending: push + confirm the first green run on GitHub, branch protection on `main` (require a PR, mark `lint` + `test` as required checks, include admins), and the Render auto-deploy-on-merge half. Deploy still on Clerk **dev** keys (private/recorded-demo path) and Tavily `search_depth="basic"`.
...
17. Docker + Render backend deploy + Vercel frontend deploy (done — live, verified end-to-end); GitHub Actions CI in progress — `.github/workflows/ci.yml` added (`lint` + `test` jobs, green locally), still to push/confirm-green + add branch protection + Render deploy-on-merge (Phase 4)
