...
- **Empty synthesizer output is a failure, not a `done` job.** `synthesizer_node` normalises `response.content` (LangChain returns a `str` *or* a list of parts — the list is joined into a string) and, if the result is empty/whitespace-only, returns `error="Synthesizer produced an empty report"` with `report_markdown=None`. Without this, a blank-but-no-exception model response would set a falsy error and persist an empty report as `done` (same gotcha as the non-empty-error note above). `call_gemini_followup` still returns `response.content` raw — coerce it too if list-shaped answers ever surface.
