...
- Test suite live (`backend/tests/`, 70 tests, green + ruff-clean — see `PRODUCTION_READINESS.md` Phase 2 and the "Testing" gotcha below). Next step: Phase 3 containerize + deploy, then Phase 4 CI (Docker + GitHub Actions + Render). **pgvector / `document_chunks` semantic chunk storage and `current_step` progress-reporting both moved to v2** — researcher continues to pass merged raw sources straight to the synthesizer (and follow-ups continue to use `raw_context`); job status polling stays coarse-grained (`pending`/`running`/`done`/`failed`) for now.
...
### Testing
- **Run from `backend/` with venv active**: `python -m pytest`. Config is `backend/pytest.ini` (`asyncio_mode = auto`, so async tests/fixtures need no per-test marker). 70 tests today, all green and ruff-clean. No external services touched — LLM/Tavily/YouTube and the DB are faked. Shared fakes + fixtures live in `tests/conftest.py`; tests split into `tests/unit/` (graph nodes, auth, helpers, routing) and `tests/api/` (endpoints via httpx `ASGITransport`).
