...
- Test suite live (`backend/tests/`, 71 tests, green + ruff-clean — see `PRODUCTION_READINESS.md` Phase 2 and the "Testing" gotcha below). Next step: Phase 3 containerize + deploy, then Phase 4 CI (Docker + GitHub Actions + Render). **pgvector / `document_chunks` semantic chunk storage and `current_step` progress-reporting both moved to v2** — researcher continues to pass merged raw sources straight to the synthesizer (and follow-ups continue to use `raw_context`); job status polling stays coarse-grained (`pending`/`running`/`done`/`failed`) for now.
