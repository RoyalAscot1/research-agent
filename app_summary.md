...
- **Rate limiting** — live via `slowapi`, keyed per Clerk user: `POST /queries` `10/hour;3/minute`, `POST /reports/{id}/followup` `20/hour;5/minute`, returning a clean 429. In-memory counters today (per-process, resets on restart); a shared Redis backend is the multi-instance upgrade. See `PRODUCTION_READINESS.md` Phase 3.
- **Cost guardrails** — no cap on Tavily credits per user; track usage and throttle at a threshold before shipping.
- **Testing** — pytest suite live (`backend/tests/`, 71 tests): graph nodes (re-plan loop, hard caps, fail-safe fallbacks), the auth dependency (incl. forged-token rejection), and the API endpoints (ownership 403 / 404 / follow-up cap 429 / per-user rate-limit 429) — all with external services and the DB faked. See `PRODUCTION_READINESS.md` Phase 2 for scope and the one deliberate gap (DB-constraint behaviour not exercised).
